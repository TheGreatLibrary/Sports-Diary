package com.sinya.projects.sportsdiary.presentation.trainingPage.bottomSheetCategory

import android.content.Context
import androidx.compose.runtime.State
import androidx.compose.runtime.mutableStateOf
import androidx.compose.ui.state.ToggleableState
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.sinya.projects.sportsdiary.data.database.repository.ExercisesRepository
import com.sinya.projects.sportsdiary.data.database.repository.TrainingRepository
import dagger.hilt.android.lifecycle.HiltViewModel
import jakarta.inject.Inject
import kotlinx.coroutines.launch



@HiltViewModel
class TrainingBottomSheetCategoryViewModel @Inject constructor(
    private val repoEx: ExercisesRepository,
    private val repoTr: TrainingRepository
) : ViewModel() {

    private val _state = mutableStateOf<TrainingBottomSheetCategoryUiState>(TrainingBottomSheetCategoryUiState.Loading)
    val state: State<TrainingBottomSheetCategoryUiState> = _state

    init {
        viewModelScope.launch {
            _state.value = TrainingBottomSheetCategoryUiState.Success(
                items = repoEx.getExercisesList().map { ExerciseUi(it.id, it.name) }
            )
        }
    }

    fun onEvent(event: TrainingBottomSheetCategoryUiEvent) {
        val currentState = _state.value as? TrainingBottomSheetCategoryUiState.Success ?: return

        when(event) {
            is TrainingBottomSheetCategoryUiEvent.OnNameChange -> {
                _state.value = currentState.copy(
                    categoryName = event.s
                )
            }
            is TrainingBottomSheetCategoryUiEvent.OnQueryChange -> {
                _state.value = currentState.copy(
                    query = event.s
                )
            }
            is TrainingBottomSheetCategoryUiEvent.ToggleExpanded -> {
                _state.value = currentState.copy(
                    expanded = !currentState.expanded
                )
            }
            is TrainingBottomSheetCategoryUiEvent.Toggle -> {
                _state.value = currentState.copy(
                    items = currentState.items.map { if (it.id == event.id) it.copy(checked = !it.checked) else it }
                )
            }
            is TrainingBottomSheetCategoryUiEvent.SetAll -> {
                _state.value = currentState.copy(
                    items = currentState.items.map { it.copy(checked = event.checked) }
                )
            }
            is TrainingBottomSheetCategoryUiEvent.CreateCategory -> {
                create(event.onDone, event.onError)
            }
        }
    }

    fun filtered() : List<ExerciseUi> {
        val currentState = _state.value as? TrainingBottomSheetCategoryUiState.Success ?: return listOf()
        return if (currentState.query.isBlank()) currentState.items else currentState.items.filter { it.name.contains(currentState.query, ignoreCase = true) }
    }
    private fun selectedIds(): List<Int> {
        val currentState = _state.value as? TrainingBottomSheetCategoryUiState.Success ?: return listOf()
        return currentState.items.filter { it.checked }.map { it.id }
    }

    fun triState(): ToggleableState {
        val currentState = _state.value as? TrainingBottomSheetCategoryUiState.Success ?: return ToggleableState.Off

        val total = currentState.items.size
        val sel = currentState.items.count { it.checked }
        return when (sel) {
            0 -> ToggleableState.Off
            total -> ToggleableState.On
            else -> ToggleableState.Indeterminate
        }
    }

    private fun create(onDone: () -> Unit, onError: (Throwable) -> Unit) {
        val currentState = _state.value as? TrainingBottomSheetCategoryUiState.Success ?: return

        val nameLocal = currentState.categoryName.trim()
        val ids = selectedIds()
        if (nameLocal.isEmpty()) {
            onError(IllegalStateException("Заполните название и выберите упражнения"))
            return
        }
        viewModelScope.launch {
            runCatching { repoTr.insertCategory(nameLocal, ids) }
                .onSuccess { onDone() }
                .onFailure { onError(it) }
        }
    }

    fun insertExercises(current: Context) {
        viewModelScope.launch {
            repoEx.importExercisesFromAssets(context = current)
        }
    }
}
